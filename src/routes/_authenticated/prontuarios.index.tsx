...
function ProntuariosIndex() {
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("patients")
        .select("id, name, species, breed, clients(full_name)")
        .order("name");
    ...
    <div className="space-y-4 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Selecione o paciente para acessar o prontuário.
        </p>
      </div>
      ...
      <div className="grid gap-2">
        ...
        {data.length === 0 && (
          <div className="text-sm text-muted-foreground text-center py-12">Nenhum paciente.</div>
        )}
      </div>
    </div>
  );
