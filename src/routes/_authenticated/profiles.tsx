  ...
  const save = useMutation({
    mutationFn: async () => {
      if (form.id) {
        ...
        await fnCreate({
          data: { name: form.name, description: form.description, modules: form.modules },
        });
      }
    },
    onSuccess: () => {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <ShieldCheck className="w-6 h-6 text-primary" /> Perfis de Acesso
          </h1>
              ...
              <div key={p.id} className="py-3 flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {p.is_system && (
                      <span className="text-[10px] uppercase tracking-wider bg-secondary text-secondary-foreground px-2 py-0.5 rounded inline-flex items-center gap-1">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {(p.modules ?? []).map((k: string) => {
                      ...
                      return (
                        <span
                          key={k}
                          className="text-[11px] px-2 py-0.5 rounded bg-primary/10 text-primary"
                        >
                          {m?.label ?? k}
                        </span>
                      );
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  {!p.is_system && (
                    <Button
                      ...
                      size="icon"
                      onClick={() => {
                        if (confirm(`Excluir o perfil "${p.name}"?`)) del.mutate(p.id);
                      }}
                    >
