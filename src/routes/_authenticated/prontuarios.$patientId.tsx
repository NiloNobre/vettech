...
import {
  printReceituarioComum,
  printReceituarioEspecial,
  printReceituarioItems,
  printAtestado,
  printExame,
  type PatientLite,
} from "@/lib/print-docs";
  ...
  const { data: patient } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("patients")
        .select("*, clients(full_name, phone, document, address)")
        .eq("id", patientId)
        .single();
    ...
    <div className="space-y-4 max-w-5xl">
      ...
      <Card>
        <CardContent className="p-5">
          ...
          <div className="text-sm mt-2">
            Tutor: <strong>{patient?.clients?.full_name}</strong>{" "}
            {patient?.clients?.phone && (
              <span className="text-muted-foreground">• {patient.clients.phone}</span>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="consultas">
        <TabsList className="flex-wrap h-auto">
          ...
          <TabsTrigger value="receituarios">
        ...
        <TabsContent value="consultas" className="space-y-3">
          ...
          {consults.map((c) => (
            <ConsultCard key={c.id} consult={c} patient={patient as PatientLite} />
          ))}
          {consults.length === 0 && (
        ...
        <TabsContent value="exames">
          <ExamesTab patient={patient as PatientLite} />
        </TabsContent>
        <TabsContent value="prescricoes">
          <PrescricoesTab patient={patient as PatientLite} />
        </TabsContent>
        <TabsContent value="receituarios">
          <ReceituariosTab patient={patient as PatientLite} />
        </TabsContent>
        <TabsContent value="atestados">
          <AtestadosTab patient={patient as PatientLite} />
        </TabsContent>
        <TabsContent value="vacinas">
          <VacinasTab patientId={patientId} />
        </TabsContent>
      </Tabs>
...
function ConsultCard({
  ...
  consult: Consult;
  patient: PatientLite | undefined | null;
}) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2 text-sm">
        {consult.anamnesis && (
          <div>
            <strong>Anamnese:</strong> {consult.anamnesis}
          </div>
        )}
        {consult.diagnosis && (
          <div>
            <strong>Diagnóstico:</strong> {consult.diagnosis}
          </div>
        )}
        {consult.treatment && (
          <div>
            <strong>Tratamento:</strong> {consult.treatment}
          </div>
        )}
        {consult.observations && (
          <div className="text-muted-foreground">{consult.observations}</div>
        )}
        {items.length > 0 && (
          <div className="mt-3 pt-3 border-t">
            <div className="font-semibold text-xs uppercase tracking-wide mb-2 flex items-center gap-1 justify-between">
              <span className="flex items-center gap-1">
                <Pill className="w-3 h-3" /> Prescrição
              </span>
              <Button
                size="sm"
                variant="outline"
                onClick={() => printReceituarioItems(patient, items)}
              >
            ...
            <ul className="space-y-1">
              {items.map((i) => (
                <li key={i.id} className="flex justify-between gap-2">
                  <span>
                    <strong>{i.product_name}</strong> {i.dosage && <>— {i.dosage}</>}{" "}
                    {i.frequency && <>• {i.frequency}</>} {i.duration && <>• {i.duration}</>}
                  </span>
                  <button
                    onClick={async () => {
                      await supabase.from("prescription_items").delete().eq("id", i.id);
                      qc.invalidateQueries({ queryKey: ["presc", consult.id] });
                    }}
                  >
...
function AddPrescription({ consultationId }: { consultationId: string }) {
  ...
  async function add() {
    ...
    if (!product) {
      toast.error("Selecione um medicamento do estoque");
      return;
    }
    setSaving(true);
  ...
  return (
    <div className="mt-3 pt-3 border-t grid gap-2">
        ...
        <div className="md:col-span-2">
          ...
          <Select value={productId} onValueChange={setProductId}>
            <SelectTrigger>
              <SelectValue
                placeholder={
                  products.length ? "Selecione um medicamento" : "Nenhum produto em estoque"
                }
              />
            </SelectTrigger>
            <SelectContent>
      ...
      <Button size="sm" onClick={add} disabled={saving || !productId}>
        ...
        {saving ? "Salvando…" : "Adicionar prescrição"}
      </Button>
    </div>
  );
}
        ...
        >
          ...
          <DialogFooter>
            <Button type="submit" disabled={save.isPending}>
              {save.isPending ? "Salvando…" : "Salvar consulta"}
            </Button>
          </DialogFooter>
        </form>
        <p className="text-xs text-muted-foreground">
          As prescrições são adicionadas após salvar a consulta, no card correspondente.
        </p>
      </DialogContent>
    </Dialog>
  );
        ...
        <CardContent className="space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Exames solicitados / orientações"
            />
          </div>
          <div>
            ...
            <CardContent className="p-4 space-y-2">
              <div className="flex justify-between items-start gap-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {format(new Date(e.date), "dd/MM/yyyy 'às' HH:mm")}
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => printExame(patient, e.name, e.requested ?? "", e.result ?? "")}
                  >
                    <Printer className="w-3 h-3 mr-1" />
                    Imprimir
                  </Button>
                  <Button
                    ...
                    variant="ghost"
                    onClick={async () => {
                      await supabase.from("exames").delete().eq("id", e.id);
                      qc.invalidateQueries({ queryKey: ["exames", patientId] });
                    }}
                  >
                    <Trash2 className="w-3 h-3 text-destructive" />
              ...
              {e.requested && (
                <div className="text-sm border-t pt-2">
                  <strong>Solicitado:</strong> {e.requested}
                </div>
              )}
              {e.result && (
...
function PrescricoesTab({ patient }: { patient: PatientLite | undefined | null }) {
  ...
  async function save() {
    ...
    const { data: presc, error } = await supabase
      .from("prescricoes")
      .insert({ patient_id: patientId, vet_id: user?.id ?? null, notes: notes || null })
      .select()
      .single();
    if (error || !presc) return toast.error(error?.message ?? "Erro");
    const { error: e2 } = await supabase
      .from("prescricao_items")
      .insert(draft.map((d) => ({ ...d, prescricao_id: presc.id })));
          ...
          <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1fr_auto] gap-2 items-end">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setItem({ ...item, product_id: v })}
              >
                <SelectTrigger>
                  <SelectValue placeholder={products.length ? "Selecione" : "Nenhum em estoque"} />
                </SelectTrigger>
                <SelectContent>
                  {products.map((p) => (
                    <SelectItem key={p.id} value={p.id}>
                      {p.name} ({p.stock} {p.unit ?? "un"})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <Input
              placeholder="Dosagem"
              value={item.dosage}
              onChange={(e) => setItem({ ...item, dosage: e.target.value })}
            />
            <Input
            ...
            />
            <Button type="button" size="sm" onClick={addItem} disabled={!item.product_id}>
            ...
            <ul className="text-sm border rounded-md divide-y">
              {draft.map((d, idx) => (
                <li key={idx} className="px-3 py-2 flex justify-between items-center">
                  <span>
                    <strong>{d.product_name}</strong> {d.dosage && <>— {d.dosage}</>}{" "}
                    {d.frequency && <>• {d.frequency}</>} {d.duration && <>• {d.duration}</>}
                  </span>
                  <button onClick={() => setDraft(draft.filter((_, i) => i !== idx))}>
                    <Trash2 className="w-3 h-3 text-destructive" />
                  </button>
                </li>
              ))}
            ...
            <CardContent className="p-4 space-y-2">
              <div className="flex justify-between items-start gap-3">
                <div className="text-sm text-muted-foreground">
                  ...
                  {p.prescricao_items?.length ?? 0} item(ns)
                </div>
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() =>
                      printReceituarioItems(patient, p.prescricao_items ?? [], p.notes ?? "")
                    }
                  >
                    <Printer className="w-3 h-3 mr-1" />
                    Imprimir
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={async () => {
                      await supabase.from("prescricoes").delete().eq("id", p.id);
                      qc.invalidateQueries({ queryKey: ["prescricoes", patientId] });
                    }}
                  >
                    <Trash2 className="w-3 h-3 text-destructive" />
              ...
              {p.notes && (
                <div className="text-xs text-muted-foreground border-t pt-2">{p.notes}</div>
              )}
            </CardContent>
          </Card>
        ))}
...
function ReceituariosTab({ patient }: { patient: PatientLite | undefined | null }) {
  ...
  async function save() {
    if (!content.trim()) {
      toast.error("Escreva o conteúdo da receita");
      return;
    }
    const {
      data: { user },
    ...
    const { error } = await supabase
      .from("receituarios")
      .insert({ patient_id: patientId, vet_id: user?.id ?? null, kind, content });
    if (error) return toast.error(error.message);
    setContent("");
              ...
              <div className="flex justify-between items-start gap-3">
                <div>
                  <div className="text-xs uppercase tracking-wide text-muted-foreground">
                    {r.kind === "especial" ? "Receita especial" : "Receita comum"}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {format(new Date(r.date), "dd/MM/yyyy 'às' HH:mm")}
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={() =>
                      r.kind === "especial"
                        ? printReceituarioEspecial(patient, r.content)
                        : printReceituarioComum(patient, r.content)
                    }
                  >
                    <Printer className="w-3 h-3 mr-1" />
                    Imprimir
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={async () => {
                      await supabase.from("receituarios").delete().eq("id", r.id);
                      qc.invalidateQueries({ queryKey: ["receituarios", patientId] });
                    }}
                  >
                    <Trash2 className="w-3 h-3 text-destructive" />
...
function AtestadosTab({ patient }: { patient: PatientLite | undefined | null }) {
  ...
  async function save() {
    if (!content.trim()) {
      toast.error("Escreva o atestado");
      return;
    }
    const {
      data: { user },
  ...
  return (
    <div className="space-y-4 pt-2">
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Novo atestado médico</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div>
              ...
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => printAtestado(patient, a.content, a.days)}
                  >
                    <Printer className="w-3 h-3 mr-1" />
                    Imprimir
                  </Button>
                  <Button
                    ...
                    variant="ghost"
                    onClick={async () => {
                      await supabase.from("atestados").delete().eq("id", a.id);
                      qc.invalidateQueries({ queryKey: ["atestados", patientId] });
                    }}
                  >
                    <Trash2 className="w-3 h-3 text-destructive" />
...
function VacinasTab({ patientId }: { patientId: string }) {
  ...
  return (
    <div className="space-y-4 pt-2">
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-base">Nova vacina aplicada</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
            ...
            <CardContent className="p-4 flex justify-between items-start gap-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  ...
                  {v.next_dose_date && (
                    <> • Próxima: {format(new Date(v.next_dose_date), "dd/MM/yyyy")}</>
                  )}
                </div>
              ...
              <Button
                ...
                variant="ghost"
                onClick={async () => {
                  await supabase.from("vacinas").delete().eq("id", v.id);
                  qc.invalidateQueries({ queryKey: ["vacinas", patientId] });
                }}
              >
                <Trash2 className="w-3 h-3 text-destructive" />
