...
interface Patient {
  id: string; client_id: string; name: string; species: string; breed: string | null;
  sex: string | null; birth_date: string | null; weight: number | null;
...
function PatientsPage() {
  ...
  const { data: patients = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from("patients").select("*, clients(full_name)").order("created_at", { ascending: false });
  ...
  const sendToQueue = useMutation({
    ...
    onSuccess: () => { toast.success("Adicionado à fila!"); qc.invalidateQueries({ queryKey: ["queue"] }); },
  ...
  return (
    <div className="space-y-4 max-w-5xl">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Button onClick={() => setOpen(true)}><Plus className="w-4 h-4 mr-2" />Novo paciente</Button>
      </div>
            ...
            <CardContent className="p-4 flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex gap-2">
                ...
                <Link to="/prontuarios/$patientId" params={{ patientId: p.id }}>
                  <Button size="sm"><ClipboardList className="w-4 h-4 mr-1" /> Prontuário</Button>
                </Link>
              </div>
            </CardContent>
          </Card>
        ))}
        {patients.length === 0 && <div className="text-sm text-muted-foreground text-center py-12">Nenhum paciente cadastrado.</div>}
      </div>
...
function PatientFormDialog({ open, setOpen }: { open: boolean; setOpen: (v: boolean) => void; }) {
  const qc = useQueryClient();
  const [form, setForm] = useState({ client_id: "", name: "", species: "Cão", breed: "", sex: "M", birth_date: "", weight: "", microchip: "" });
  ...
  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("patients").insert({
        client_id: form.client_id, name: form.name, species: form.species,
        breed: form.breed || null, sex: form.sex || null,
    ...
    onSuccess: () => {
      ...
      setOpen(false);
      setForm({ client_id: "", name: "", species: "Cão", breed: "", sex: "M", birth_date: "", weight: "", microchip: "" });
    },
    onError: (e: Error) => toast.error(e.message),
  });
        ...
        <form onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="space-y-3">
          <div>
            ...
            <Select value={form.client_id} onValueChange={(v) => setForm({ ...form, client_id: v })}>
              <SelectTrigger><SelectValue placeholder="Selecione um tutor" /></SelectTrigger>
              <SelectContent>
                {clients.map((c) => <SelectItem key={c.id} value={c.id}>{c.full_name}</SelectItem>)}
              </SelectContent>
            </Select>
            {clients.length === 0 && <p className="text-xs text-muted-foreground mt-1">Cadastre um cliente primeiro.</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select value={form.species} onValueChange={(v) => setForm({ ...form, species: v })}>
                ...
                <SelectContent>
                  {["Cão", "Gato", "Ave", "Roedor", "Réptil", "Outro"].map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
          ...
          <div><Label>Microchip</Label><Input value={form.microchip} onChange={(e) => setForm({ ...form, microchip: e.target.value })} /></div>
          <DialogFooter><Button type="submit" disabled={save.isPending || !form.client_id}>{save.isPending ? "Salvando…" : "Cadastrar"}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
