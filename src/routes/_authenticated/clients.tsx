  ...
  return (
    <div className="space-y-4 max-w-5xl">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="w-4 h-4 mr-2" />Novo cliente</Button>
      </div>
          ...
          <Card key={c.id}>
            <CardContent className="p-4 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-1">
                ...
                <Button size="icon" variant="ghost" onClick={() => { if (confirm("Remover cliente?")) del.mutate(c.id); }}><Trash2 className="w-4 h-4 text-destructive" /></Button>
              </div>
            </CardContent>
          </Card>
        ))}
        {clients.length === 0 && <div className="text-sm text-muted-foreground text-center py-12">Nenhum cliente cadastrado.</div>}
      </div>
...
function ClientFormDialog({ open, setOpen, client }: { open: boolean; setOpen: (v: boolean) => void; client: ClientRow | null; }) {
  const qc = useQueryClient();
  const isEdit = !!client;
  ...
  const save = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        document: form.document || null, address: form.address || null, notes: form.notes || null,
      };
      if (isEdit) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setForm({ full_name: "", email: "", phone: "", document: "", address: "", notes: "" }); }}>
      <DialogContent className="max-w-lg">
        ...
        <form onSubmit={(e) => { e.preventDefault(); save.mutate(); }} className="space-y-3">
          ...
          <div><Label>CPF / Documento</Label><Input value={form.document} onChange={(e) => setForm({ ...form, document: e.target.value })} /></div>
