...
function Reports() {
  ...
  function exportPdf() {
    ...
    const days = Object.entries(data.byDay)
      .sort()
      .map(([d, n]) => `<tr><td>${d}</td><td style="text-align:right">${n}</td></tr>`)
      .join("");
    const html = `
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat
          label="Na fila"
          value={(data?.queueByStatus.waiting ?? 0) + (data?.queueByStatus.called ?? 0)}
        />
      </div>
        ...
        <CardContent>
          <div className="space-y-1 text-sm">
            ...
            {!Object.keys(data?.byDay ?? {}).length && (
              <div className="text-muted-foreground">Sem atendimentos.</div>
            )}
          </div>
        </CardContent>
      </Card>
