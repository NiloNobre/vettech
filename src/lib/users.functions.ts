  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: profilesData }, { data: assigns }] = await Promise.all([
      ...
      supabaseAdmin
        ...
        .select("user_id, profile_id, access_profiles(name)")
        .in("user_id", safeIds),
    ]);
    ...
    const assignMap = new Map(
      (assigns ?? []).map((a: any) => [
        a.user_id,
        { profile_id: a.profile_id, profile_name: a.access_profiles?.name ?? null },
      ]),
    );
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.is_admin) {
      ...
      if (data.user_id === context.userId)
        throw new Error("Não é possível remover o próprio acesso de administrador.");
      await supabaseAdmin
        .from("user_roles")
        .delete()
